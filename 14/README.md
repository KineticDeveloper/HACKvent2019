...
https://academy.hacking-lab.com/events/6/challenges/75

Today we have to run and crack a perl script:
...
Running the code on a Linux box with `perl-tk` installed, shows a game where one has to catch the flag with the orange dot, not touching the borders or crossing an orange line. The game is too hard to play, so it is time to modify it.

The code can be formatted for easier reading using an (online Perl formatter)[https://www.tutorialspoint.com/online_perl_formatter.htm].

I've made the following modifications:

- slow done the speed.
- ensure the flags are directly generated infront of the player. So it takes only seconds to play :)
- log the generated flag by modifying `$d28Vt03MEbdY0`
...
After executing the game with `perl game.pl > code.txt` the resulting flag:
